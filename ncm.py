    ...
    if len(sys.argv) > 1:
        ...
        for ncm_file in files:
            if ncm_file.endswith(".ncm"):
                converted_count += 1
                dump(path + os.sep + ncm_file)
                print("No ", converted_count, " converting file ", ncm_file)

    else:
        print("Usage: python ncm_dump.py \"directory Name\"")

    print(total, " files, ", converted_count, " are converted")
